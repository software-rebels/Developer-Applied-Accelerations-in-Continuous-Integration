...
{
    public async Task<IActionResult> ByStepNumber(string repo, string job, string branch = "", string why = "", string where = "",
            [FromServices] ForecastingContext forecastingContext = default!)
    {
        ...
        var (maxBranch, count) = builds.TakeLast(refBuildCount)
            .Aggregate(new Dictionary<string, nint>(),
                (d, b) => { d[b.Branch ?? string.Empty] = CollectionExtensions.GetValueOrDefault<string, nint>(d, b.Branch ?? string.Empty) + 1; return d; })
            .MaxBy<KeyValuePair<string, nint>, nint>((Func<KeyValuePair<string, nint>, nint>)(d => d.Value));
    ...
    private static ArraySegment<T> CreateSegment<T>(T[] array) => array;
}
